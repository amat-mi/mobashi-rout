##########################
# Copied from: https://github.com/herrkaefer/psycopgr/blob/master/psycopgr/psycopgr.py
# Started updating it to newer version of pgRouting.
# Should be forked and a Pull Request should be created when finished updating.
##########################
...
class PGRouting(object):
    ...
    @property
    def cursor(self):
        if (self._cur is None or self._cur.closed) or (self._conn is None or self._conn.closed):
            self._connect_to_db(*self._args, **self._kwargs)
        return self._cur
    ...
    def _find_nearest_vertices(self, nodes: List[PgrNode]) -> List[PgrNode]:
        ...
        for node in nodes:
            try:
                self.cursor.execute(sql, (node.lon, node.lat))
    ...
    def dijkstra_cost(self, start_vids, end_vids):
        ...
        try:
            self.cursor.execute(sql, (start_vids, end_vids))
            results = self.cursor.fetchall()
    ...
    def astar(self, start_vid, end_vid):
        ...
        try:
            self.cursor.execute(sql, (start_vid, end_vid))
            results = self.cursor.fetchall()
